...
class GraphAdjDataset(data.Dataset):
    def __init__(self, data=None, k=2):
        ...
        if data:
            self.data = GraphAdjDataset.preprocess_batch(data, k, use_tqdm=False)
        else:
            self.data = list()
        self._to_tensor()
    ...
    def preprocess(x, k):
        ...
        graph_dglgraph.edata["label"] = torch.from_numpy(np.array([0]*graph_dglgraph.num_edges(), dtype=np.int64))
        ...
        # random_rotio = 0.1
        # random_subgraph_list = random.sample(subgraph_list, max(1, int(len(subgraph_list) * random_rotio)))
